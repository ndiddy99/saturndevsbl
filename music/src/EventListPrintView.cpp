...
BEGIN_MESSAGE_MAP (CEventListPrintView, CSekaijuView)
	ON_COMMAND (ID_FILE_PRINT, CSekaijuView::OnFilePrint)
	ON_COMMAND (ID_FILE_PRINT_DIRECT, CSekaijuView::OnFilePrint)
	ON_COMMAND (ID_FILE_PRINT_PREVIEW, CSekaijuView::OnFilePrintPreview)
END_MESSAGE_MAP ()
...
CString CEventListPrintView::GetCellString (long lRow, long lColumn) {
	...
	case 0:
		...
		codestr2str (szBuf1, TCSLEN (szBuf1), szBuf2, TSIZEOF (szBuf2));
...
void CEventListPrintView::DrawIndexScaleView (CDC* pDC, CPrintInfo* pInfo) {
	...
	CFont* pOldFont = pDC->SelectObject (&(pSekaijuApp->m_theFont[2]));
	long lRowZoom = 40;
	long lVisibleEventCount = pEventListFrame->GetVisibleEventCount ();
...
void CEventListPrintView::OnPrint (CDC* pDC, CPrintInfo* pInfo) {
	...
	pOldFont = pDC->SelectObject (&(pSekaijuApp->m_theFont[2]));
	pDC->DrawText (strText, rcText, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
	pDC->SelectObject (pOldFont);
